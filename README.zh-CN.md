...
安装器会把插件复制到个人 marketplace，注册在 Codex 重启后继续保持透明度的 Windows 登录任务；如果 Codex CLI 可用，还会自动注册 `codex-window-opacity@personal`。已有安装可重新运行安装器更新。
...
支持的不透明度范围是 `35` 到 `100`，默认值为 `90`。Codex 窗口重建后会自动重新应用保存的数值。
...
卸载器会停止 Windows 任务、尝试恢复完全不透明，然后移除本插件及其 marketplace 条目。若想保留透明度设置，请添加 `-KeepSettings`。
...
- 如果保存值没有自动应用，请检查 Windows 计划任务 `CodexWindowOpacity` 是否正在运行。
